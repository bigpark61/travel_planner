...
def plan_route():
    ...
    try:
        ...
        return jsonify({"error": 'date must use YYYY-MM-DD, for example "2026-09-16"'}), 400

    tp.reset_errors()
    dry = request.args.get("dry_run", "0")
    tp.DRY_RUN = dry.lower() in ("1", "true", "yes")
    use_cache = request.args.get("use_cache", "0").lower() in ("1", "true", "yes")
    if use_cache:
        cached = tp.load_cached_results(date)
        if cached:
            recommendation, restaurants_by_city, report_md = cached
            return jsonify({
                "cached": True,
